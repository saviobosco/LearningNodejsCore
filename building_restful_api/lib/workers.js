 ...
 */
 ...
 workers.gatherAllChecks = function() {
    // get all checks that exist on the system
 ...
 workers.validateCheckData = function(originalCheckData) {
     ...
     // Set the keys that may not be set if the workers has not seen the check before
 ...
 // perform the check, send the originalCheckData and the outcome to the next process
 workers.performCheck = function(originalCheckData) {
     ...
     var path = parsedUrl.path // we are using path and not pathname because we want the full query string that the user specified.
     ...
     var _moduleToUse = originalCheckData == "http" ? http : https;
 ...
 // Process the checkOutcome, update the check data as needed, trigger an lert to the user
 // Special logic for accomodating a check that has never been tested before and dont want to alert that one
        ...
        if(!err && logs && logs.length > 0) {
            logs.forEach(function(logName) {
                ...
                _logs.compress(logId, newFileId, function(err) {
                    if (!err) {
                        // Truncate the log or Emptying the log file
 ...
 //Timer to rotate the log-rotation process once per day
